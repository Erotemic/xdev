...
class InteractiveShellEmbedEnhanced(InteractiveShellEmbed):
    ...
    def init_frame(self, frame):
        if frame.f_code.co_name == '<module>':  # if embedded outside a function
            ...
            self._ns['_ih'] = self.history_manager.input_hist_parsed  # type: ignore
            self._ns['_oh'] = self.history_manager.output_hist  # type: ignore
            self._ns['_dh'] = self.history_manager.dir_hist  # type: ignore
