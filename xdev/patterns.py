...
class Pattern(PatternBase, ub.NiceRepr):
    ...
    def to_regex(self):
        ...
        if self.backend == 'regex':
            ...
            regex_pattern = self.pattern._expression  # type: ignore
        elif self.backend == 'glob':
            regex_pattern = fnmatch.translate(self.pattern)  # type: ignore
        elif self.backend == 'strict':
            regex_pattern = re.escape(self.pattern)  # type: ignore
        else:
    ...
    def match(self, text):
        ...
        if self.backend == 'regex':
            return self.pattern.match(text)  # type: ignore
        elif self.backend == 'parse':
            return self.pattern.parse(text)  # type: ignore
        elif self.backend == 'glob':
            return fnmatch.fnmatch(text, self.pattern)  # type: ignore
        elif self.backend == 'strict':
    ...
    def search(self, text):
        if self.backend == 'regex':
            ...
            return fnmatch.fnmatch(text, '*{}*'.format(self.pattern))  # type: ignore
        elif self.backend == 'strict':
    ...
    def sub(self, repl, text, count=-1):
        ...
        if self.backend == 'regex':
            return self.pattern.sub(repl, text, count=max(0, count))  # type: ignore
        elif self.backend == 'parse':
            raise NotImplementedError
        elif self.backend == 'glob':
            raise NotImplementedError
